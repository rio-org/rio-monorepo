...
import {MockBeaconChainOracle} from 'test/utils/beacon-chain/MockBeaconChainOracle.sol';
import {BeaconChainProofs} from 'test/utils/beacon-chain/BeaconChainProofs.sol';
import {Merkle} from 'test/utils/beacon-chain/Merkle.sol';
...
contract MockBeaconChain is Test {
    ...
    MockBeaconChainOracle oracle;

    /// @dev All withdrawals are processed with index == 0
    uint64 constant WITHDRAWAL_INDEX = 0;
    uint256 constant GWEI_TO_WEI = 1e9;

    constructor(MockBeaconChainOracle beaconChainOracle) {
        nextTimestamp = uint64(block.timestamp);
        oracle = beaconChainOracle;
    }

    /// @dev Sets the next timestamp to be used for proof generation.
    ...
    function newValidator(uint256 balanceWei, bytes memory withdrawalCreds)
        public
        returns (uint40, CredentialsProofs memory)
    {
        ...
        uint256 depositAmount = balanceWei / GWEI_TO_WEI;
    ...
    function exitValidator(uint40 validatorIndex) public returns (BeaconWithdrawal memory) {
        ...
        uint256 amountToWithdraw = validator.effectiveBalanceGwei * GWEI_TO_WEI;
    ...
    ///
    /// The generated block root is sent to the `MockBeaconChainOracle`, and can be
    ...
    function _genCredentialsProof(Validator memory validator) internal returns (CredentialsProofs memory) {
        ...
        uint256 index = _calcValProofIndex(validator.validatorIndex);
        ...
        proof.stateRootProof =
            IBeaconChainProofs.StateRootProof({beaconStateRoot: beaconStateRoot, proof: blockRootProof});
    ...
    /// the beacon chain.
    ///
    /// The generated beacon block root is sent to `MockBeaconChainOracle`, and can
    ...
    function _genExitProof(Validator memory validator) internal returns (BeaconWithdrawal memory) {
...

        {
            ...
            withdrawalProof.blockRoot = _genBlockRootProofs({withdrawalProof: withdrawalProof});
        }
    ...
    function _genBlockRootProofs(IBeaconChainProofs.WithdrawalProof memory withdrawalProof)
        ...
        returns (bytes32)
    {
        uint256 slotRootIndex = BeaconChainProofs.SLOT_INDEX;
        uint256 execPayloadIndex = (
    ...
    function _genBeaconStateRootProofs(
        ...
        uint256 blockHeaderIndex = _calcBlockHeaderIndex(withdrawalProof);
        uint256 validatorProofIndex = _calcValProofIndex(validatorIndex);
    ...
    function _genConvergentProofs(
        ...
        uint256 longProofOffset = longProof.length - shortProof.length;
        for (uint256 i = 32; i <= longProofOffset; i += 32) {
            if (longIndex % 2 == 0) {
                assembly {
            ...
            assembly {
                if iszero(staticcall(sub(gas(), 2000), 2, 0x00, 0x40, curLongHash, 0x20)) { revert(0, 0) }
...

        {
            ...
            uint256 longProof_i = 32 + longProofOffset;
            uint256 shortProof_i = 32;
            ...
            for (; longProof_i <= longProof.length;) {
                ...
                // Compute next hash for shortProof
                {
                    ...
                    // Compute hash and divide index
                    assembly {
                        if iszero(staticcall(sub(gas(), 2000), 2, 0x00, 0x40, curShortHash, 0x20)) { revert(0, 0) }
    ...
    uint256 immutable BLOCKROOT_PROOF_LEN = 32 * BeaconChainProofs.BEACON_BLOCK_HEADER_FIELD_TREE_HEIGHT;
    uint256 immutable VAL_FIELDS_PROOF_LEN =
        32 * ((BeaconChainProofs.VALIDATOR_TREE_HEIGHT + 1) + BeaconChainProofs.BEACON_STATE_FIELD_TREE_HEIGHT);

    uint256 immutable WITHDRAWAL_PROOF_LEN = 32
        * (BeaconChainProofs.EXECUTION_PAYLOAD_HEADER_FIELD_TREE_HEIGHT + BeaconChainProofs.WITHDRAWALS_TREE_HEIGHT + 1);
    uint256 immutable EXECPAYLOAD_PROOF_LEN = 32
        * (BeaconChainProofs.BEACON_BLOCK_HEADER_FIELD_TREE_HEIGHT + BeaconChainProofs.BEACON_BLOCK_BODY_FIELD_TREE_HEIGHT);
    uint256 immutable SLOT_PROOF_LEN = 32 * (BeaconChainProofs.BEACON_BLOCK_HEADER_FIELD_TREE_HEIGHT);
    uint256 immutable TIMESTAMP_PROOF_LEN = 32 * (BeaconChainProofs.EXECUTION_PAYLOAD_HEADER_FIELD_TREE_HEIGHT);
    uint256 immutable HISTSUMMARY_PROOF_LEN = 32
        * (
            BeaconChainProofs.BEACON_STATE_FIELD_TREE_HEIGHT + BeaconChainProofs.HISTORICAL_SUMMARIES_TREE_HEIGHT
                + BeaconChainProofs.BLOCK_ROOTS_TREE_HEIGHT + 2
        );

    uint256 immutable HIST_SUMMARIES_PROOF_INDEX = BeaconChainProofs.HISTORICAL_SUMMARIES_INDEX
        << (BeaconChainProofs.HISTORICAL_SUMMARIES_TREE_HEIGHT + 1 + BeaconChainProofs.BLOCK_ROOTS_TREE_HEIGHT + 1);
    ...
    function _calcBlockHeaderIndex(IBeaconChainProofs.WithdrawalProof memory withdrawalProof)
        ...
        returns (uint256)
    {
        return HIST_SUMMARIES_PROOF_INDEX
            | (uint256(withdrawalProof.historicalSummaryIndex) << (BeaconChainProofs.BLOCK_ROOTS_TREE_HEIGHT + 1))
            | (BeaconChainProofs.BLOCK_SUMMARY_ROOT_INDEX << BeaconChainProofs.BLOCK_ROOTS_TREE_HEIGHT)
            | uint256(withdrawalProof.blockRootIndex);
    }

    function _calcValProofIndex(uint40 validatorIndex) internal pure returns (uint256) {
        return (BeaconChainProofs.VALIDATOR_TREE_ROOT_INDEX << (BeaconChainProofs.VALIDATOR_TREE_HEIGHT + 1))
    ...
    /// [A, B] or [B, A]
    function _areSiblings(uint256 a, uint256 b) internal pure returns (bool) {
