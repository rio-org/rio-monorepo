...
contract MockSTETH is IERC20 {
    ...
    /// Mint shares to the `caller` to prevent a division by zero error later on.
    constructor() payable {
    ...
    /// @notice Mint shares with Ether.
    function submit(address) public payable returns (uint256 _sharesAmount) {
        require(msg.value != 0, 'ZERO_DEPOSIT');
    ...
    /// @return data the total amount of shares in existence.
    function _getTotalShares() internal view returns (uint256 data) {
        assembly {
            data := sload(TOTAL_SHARES_POSITION)
        }
    }
    ...
    function _mintShares(address _recipient, uint256 _sharesAmount) internal returns (uint256 newTotalShares) {
        ...
        assembly {
            sstore(TOTAL_SHARES_POSITION, newTotalShares)
        }
