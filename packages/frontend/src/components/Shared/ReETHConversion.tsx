...
const ReETHConversion = () => {
  ...
  return (
    <button
      className='group px-4 py-2 flex flex-row justify-center items-center gap-2 border border-[var(--color-element-wrapper-bg-light)] hover:border-[var(--color-element-wrapper-bg-light-hover)] rounded-full bg-transparent text-black text-[10px] hover:bg-[var(--color-element-wrapper-bg-hover)] hover:text-black'
      onClick={() => handleChange()}
    >
      <span className='text-[var(--color-black-50)] group-hover:text-black'>{first} = {second}</span>
      <motion.div
        animate={{ rotate: isReEth ? 180 : -180 }}
        transition={{ duration: 0.2 }}
      >
        <Image src={iconTransaction} width={12} height={12} alt=""
