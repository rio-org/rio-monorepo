...
const RestakeForm = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      setTokenSymbol(data?.symbol as TokenSymbol);
    }
  }, [data]);

  if (isError) return <Alert color="red">Error loading account balance.</Alert>
  if (isLoading) return <div className='w-full text-center min-h-[100px] flex items-center justify-center'><Spinner /></div>

  return (
    <>
      <StakeField amount={amount} accountTokenBalance={accountTokenBalance} tokenSymbol={tokenSymbol} setAmount={setAmount} />
